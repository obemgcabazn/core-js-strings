...
function removeFirstOccurrences(str, value) {
  const found = str.indexOf(value);
  if (found !== -1) {
    return str.slice(0, found) + str.slice(found + value.length);
  }
  return str;
}
...
function invertCase(str) {
  return str
    .split('')
    .map((n) => {
      return n.charCodeAt(0) === n.toLowerCase().charCodeAt(0)
        ? n.toUpperCase()
        : n.toLowerCase();
    })
    .join('');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    encoded += index + 1 ? output[index] : str[i];
  }
  return encoded;
}
